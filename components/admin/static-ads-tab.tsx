...
export default function StaticAdsTab() {
  ...
  const fetchAds = async () => {
    try {
      setIsLoading(true)
      const response = await fetch("/api/admin/static-ads")
      ...
      toast({
        title: "Error",
        description: "Failed to load static ads",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const url = "/api/admin/static-ads"
      ...
      if (data.success) {
        toast({
          title: "Success",
          description: editingAdId ? "Ad updated successfully" : "Static ad uploaded successfully",
        })
        setFormData({ 
          companyName: "", 
          description: "", 
  ...
  const handleDelete = async () => {
    ...
    try {
      setIsDeleting(true)
      const response = await fetch(`/api/admin/static-ads?adId=${deleteAdId}`, {
        method: "DELETE",
      })
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>{editingAdId ? "Edit Static Ad" : "Upload New Static Ad"}</CardTitle>
            {editingAdId && (
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                id="description"
                placeholder="Enter advertisement description (for internal reference)"
                value={formData.description}
                onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                rows={3}
              />
              <p className="text-xs text-muted-foreground">
                Note: Only the poster will be visible to users. Description is for admin reference.
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">
                Users will be redirected here when they click the ad poster
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              {formData.posterUrl ? (
                <div className="space-y-3">
                  <div className="relative aspect-[16/9] rounded-lg overflow-hidden bg-gray-100 border-2 border-green-500 max-w-md">
                    <Image
                    ...
                    {isUploadingImage ? (
                      ...
                      <>
                        ...
                        <p className="text-xs font-semibold text-purple-600">
                          Recommended size: 728x90px (Banner) or 300x250px (Medium Rectangle)
                        </p>
                      </>
                    )}
                  </label>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-10">
              <p className="text-muted-foreground">No static ads uploaded yet</p>
            </div>
          ) : (
